...
sub get_engine_command {
    ...
    } elsif ($opt_engine_type eq "jsc") {
        &dd ("getting jsc engine command.");
        $retval = &get_jsc_engine_command;
    } else {
...
#
# get the built in macOS jsc engine from the jsc framework
sub get_jsc_engine_command {
    return "/System/Library/Frameworks/JavaScriptCore.framework/Versions/Current/Resources/jsc"
}
